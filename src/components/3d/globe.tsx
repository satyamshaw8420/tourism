...
function EarthSphere({ colorTheme }: { colorTheme: keyof typeof colorThemes }) {
  ...
  const earthTexture = useMemo(() => {
    ...
    // Create a more detailed earth-like texture with theme colors
    const gradient = ctx.createLinearGradient(0, 0, 0, 512)
    gradient.addColorStop(0, theme.ocean) // Deep ocean
    gradient.addColorStop(0.2, theme.ocean) // Ocean
    gradient.addColorStop(0.4, theme.land) // Land
    gradient.addColorStop(0.6, theme.land) // Land
    gradient.addColorStop(0.8, theme.land) // Land
    gradient.addColorStop(1, theme.ocean) // Deep ocean
    
    ctx.fillStyle = gradient
    ctx.fillRect(0, 0, 1024, 512)
    
    // Add some continent-like shapes with theme colors
  ...
  return (
    <group>
      ...
      {/* Atmosphere glow with theme color */}
